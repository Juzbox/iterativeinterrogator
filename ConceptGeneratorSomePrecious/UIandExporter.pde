...
//Press D to export all the permutations into a text file
void download () {
      ...
      for (int j=0; j<methodList.size(); j++) {
        for (int k=0; k<userList.size(); k++) {
          for (int l=0; l<stateList.size(); l++) {
            ...
            output.println("Do you think "+ adjectiveList.get(i) +" "+ userList.get(k) + " would ever " + stateList.get(l) + " when "+ methodList.get(j)+"?");
          }
        }
      }
    //}
